...
| contextsafe import | registered format name, one caller-owned source, case document, checkpoint | observation-set document for `evaluate --observations`, whole or nothing; reference-only ungoverned mapping, `profile_reviewed` always false; never persists, copies, indexes, logs, or checks a plan (B-022); `lis-csv` and `lis-json` read a laboratory export at `lis_return`: the identity columns become observations, and a row carrying the whole result column set additionally becomes one ungoverned laboratory result observation, which no command writes and no receipt carries (B-025, B-030) |
