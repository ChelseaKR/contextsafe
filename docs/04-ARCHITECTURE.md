...
| contextsafe import | registered format name, one caller-owned source, case document, checkpoint | observation-set document for `evaluate --observations`, whole or nothing; reference-only ungoverned mapping, `profile_reviewed` always false; never persists, copies, indexes, logs, or checks a plan (B-022) |
