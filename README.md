...
| Security & Supply-Chain | Applies — Semgrep SAST, gitleaks secret scan at three scopes (pre-commit diff, full-history CI gate via `make secret-scan`, and the same scan again at a release tag), pip-audit dependency audit (`.github/workflows/security.yml` + `make audit`), pinned `uv.lock`, SHA-pinned actions kept current by Dependabot (`.github/dependabot.yml`, weekly, 7-day cooldown), `SECURITY.md` |
