...
| Release & Versioning | Applies — tag-triggered `release.yml` re-runs the full-history secret scan and `make verify` at the tag, gates on a matching CHANGELOG section, and builds an sdist and a wheel it does not publish or sign. No tag and no release exist yet, so it has never fired. `package.yml` triggers on the same tag shape **and on `workflow_dispatch`**, and has never fired because neither of its two triggers has happened — not because no tag exists. `CHANGELOG.md` carries a dated `## [0.1.0]` section and `CITATION.cff` carries `version: "0.1.0"` and `date-released: 2026-09-02`, both written before the tag because the release job refuses to build without the heading. Whether the tag follows, and at which version, is open — the evidence is assembled in [open decisions §1](docs/OPEN-DECISIONS.md) |
