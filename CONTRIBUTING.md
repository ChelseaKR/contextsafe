...
| Published patterns | `make patterns` | every `pattern` in `schemas/` against the constants the runtime compiles: equal to one, derived from named ones by a function the gate recomputes, or declared as having none with the reason printed on every run. A published pattern in none of the three is a finding, and so is a derivation or declaration that matches nothing published. Exit 2 when it read no contract, because a grammar gate that examined nothing is not a clean one. |
